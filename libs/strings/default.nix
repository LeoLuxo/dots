...
rec {
  ...
  */
  toPascalCase = string: lib.concatMapStrings toUpperCaseFirstLetter (splitWords string);
  ...
  */
  toPascalCaseWithSpaces =
    string: lib.concatMapStringsSep " " toUpperCaseFirstLetter (splitWords string);
  ...
  */
  sanitizePath =
    ...
    builtins.path {
      inherit path;
      name = lib.sanitizeDerivationName (lib.baseNameOf path);
    };
  ...
  */
  replaceScriptVariables =
    script: variables:
    let
      varNames1 = lib.map (x: "$" + x) (lib.attrNames variables);
      varNames2 = lib.map (x: "\${" + x + "}") (lib.attrNames variables);
